...
/**
 * Mimicks the pai-client but provider mock data functionality
 * to provide Sergio with uninterrupted access to his GUI and faster testing locally.
 * 
 * 
 */
export class PAIClient {
    ...
    async downloadReport(reportName) {
        const fileName = `${path.resolve(path.dirname(''))}/../papi/${reportName.replace(/[ ]+/g,"_").toLocaleLowerCase()}-MOCK.json`;
        //console.log(`mock client opening ${fileName}`);
        return await dfd.read_json(fileName)
            .then(df => {
                return df;
            }).catch(err=>{
                console.log(`ERROR: PAI Mock client requested "${reportName}" but no local copy was found`, err);
            })
